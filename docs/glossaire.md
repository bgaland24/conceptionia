---
titre: Glossaire métier
statut: a_valider
version: 0.2
derniere_modification: 2026-04-02
modifie_par: session-001
resume_modifications: |
  Enrichissement complet depuis description.md :
  - Ajout de tous les acronymes de la liste officielle
  - Ajout des termes métier issus des sections 1.3 et 1.4
---

# Glossaire métier

!!! warning "Statut : À valider"
    Ce document est en attente de relecture et validation par les sachants métier.

---

## Acronymes officiels

| Acronyme | Signification |
|---|---|
| AMOA | Assistance à Maîtrise d'Ouvrage |
| API | Application Programming Interface |
| BI | Business Intelligence |
| CI/CD | Continuous Integration / Continuous Deployment |
| COP | Contrat d'Objectifs et de Performance |
| COPIL | Comité de Pilotage |
| COPROJ | Comité Projet |
| CRM | Customer Relationship Management |
| DHS | Distinction, Homogénéité et Stabilité |
| DQ | Direction de la Qualité |
| DSI | Direction des Systèmes d'Information |
| ERP | Enterprise Resource Planning |
| ESB | Enterprise Service Bus |
| ESN | Entreprise de Services Numériques |
| ESTA | European Seed Treatment Assurance |
| ETL | Extract, Transform, Load |
| GED | Gestion Électronique de Documents |
| GNIS | Groupement National Interprofessionnel des Semences et plants |
| GSPP | Guide des Standards de Production et de Protection |
| LDAP | Lightweight Directory Access Protocol |
| MCD | Modèle Conceptuel de Données |
| MDM | Master Data Management |
| MPD | Modèle Physique de Données |
| MRV | Matériel de Reproduction Végétal |
| PAQS | Plan d'Assurance Qualité et Sécurité |
| PGRP | Plan de Gestion des Risques Phytosanitaires |
| PQP | Plan de Qualité Produit |
| PWA | Progressive Web App |
| RSE | Responsabilité Sociétale des Entreprises |
| SI | Système d'Information |
| SOCFrance | Service Officiel de Contrôle France |
| TIME | Tolerate – Invest – Migrate – Eliminate (méthode Gartner) |

---

## Termes métier

### A

**Agent administratif**
: Collaborateur Entreprise X intervenant dans les phases de traitement des demandes, en effectuant des vérifications administratives. Peut être situé en antenne régionale ou au niveau central.

**Agrément**
: Autorisation formelle délivrée par la DQ à un laboratoire ou à un personnel d'entreprise, attestant de sa compétence reconnue pour réaliser des auto-contrôles. Distinct de l'autorisation à produire qui s'adresse à la personne morale.

**Analyse de risque**
: Évaluation permettant de différencier les pressions de contrôle à appliquer selon le niveau de risque d'une situation (zone géographique, groupe d'espèce) ou selon l'indice de confiance d'un opérateur.

**Antenne régionale**
: L'une des 6 implantations régionales de la DQ, encadrée par un manager de proximité pouvant cumuler la fonction de référent technique régional.

**Attestation officielle**
: Document délivré sur demande d'un opérateur, attestant officiellement d'un fait ou d'une conformité constatée.

**Auto-contrôle**
: Vérification réalisée par l'opérateur lui-même (ou par son personnel agréé) sur sa propre production, dans le cadre d'un dispositif reconnu et soumis à surveillance officielle.

**Autorisation à produire**
: Agrément permettant à un opérateur (personne morale ou physique) de multiplier, produire et commercialiser des semences et plants dans le respect des normes de qualité. L'un des 5 programmes-cadres du COP.

### C

**Certificat à l'exportation**
: Document officiel requis pour l'exportation de semences vers des pays tiers ou dans le cadre de circuits soumis à surveillance officielle. Deux modalités : inspection officielle obligatoire (pays tiers) ou auto-contrôles sous surveillance officielle.

**COP (Contrat d'Objectifs et de Performance)**
: Contrat signé entre Entreprise X et l'État définissant les 5 programmes-cadres des missions officielles de contrôle : Autorisation PP, Autorisation à produire, Inspection des cultures, Inspection des lots, Reconnaissance des laboratoires.

**Contrôle officiel**
: Vérification réalisée par un agent habilité d'Entreprise X, permettant d'attester de la conformité d'un opérateur, d'un lot ou d'une culture aux normes réglementaires.

**Contrôle variétal**
: Vérification de la conformité variétale d'un lot ou d'une culture, souvent associé au contrôle DHS.

### D

**Décision**
: Acte administratif signé clôturant un processus de contrôle. Peut avoir un impact sur les droits et autorisations d'un opérateur, les agréments de personnes physiques ou de laboratoires, et l'indice de confiance de l'opérateur.

**Déclaration de matière**
: Obligation déclarative d'un opérateur portant sur les matières (semences, plants) qu'il produit, transforme ou commercialise.

**DHS (Distinction, Homogénéité, Stabilité)**
: Critères d'évaluation d'une variété végétale permettant son inscription au catalogue officiel. Fait l'objet d'une activité de contrôle dédiée.

**DQ (Direction de la Qualité)**
: Direction d'Entreprise X organisée en 3 pôles et 6 antennes régionales, en charge de faire appliquer les réglementations techniques concernant la production, le contrôle et la certification des semences et plants. Seule la Directrice de la DQ est habilitée à organiser le pouvoir de décision.

**Dossier**
: Regroupement de toutes les pièces et étapes relatives à une demande ou à un contrôle pour un opérateur donné.

### E

**ESTA (European Seed Treatment Assurance)**
: Référentiel professionnel européen certifiant les pratiques de traitement des semences. Souvent associé au PQP.

### G

**GSPP (Guide des Standards de Production et de Protection)**
: Référentiel professionnel de bonnes pratiques encadrant la production de semences et plants.

### I

**Indice de confiance**
: Indicateur attribué à un opérateur, reflétant son historique de conformité aux contrôles officiels. Influence directement la pression de contrôle appliquée (analyse de risque).

**Inspecteur**
: Agent Entreprise X en charge de la réalisation des contrôles terrain, de l'élaboration des rapports et des décisions de contrôle et surveillance, hors autorisations des systèmes de production.

### L

**Label Rouge**
: Signe officiel français de qualité supérieure applicable à certains produits semenciers. Fait l'objet d'une activité de contrôle dans les référentiels professionnels.

**Lot de semences**
: Quantité définie de semences d'une même espèce/variété, produite dans des conditions homogènes et identifiable par un numéro de lot pour le contrôle.

### M

**Manager de proximité**
: Responsable encadrant un pôle ou une antenne régionale. Peut cumuler la fonction de référent technique régional pour les antennes.

**MRV (Matériel de Reproduction Végétal)**
: Ensemble des semences, plants et autres matériaux utilisés pour la reproduction des végétaux.

### O

**Objectif de contrôle**
: Traduction opérationnelle d'un programme-cadre COP en cible quantifiée et qualifiée d'activités de contrôle à réaliser sur une période.

**Opérateur**
: Personne morale ayant au moins une activité en lien avec le secteur semences et plants (multiplication, production, commercialisation). Soumis à obligations déclaratives et à contrôles officiels.

**Ordonnancement**
: Étape du cycle de contrôle consistant à émettre des ordres de contrôle en réponse aux demandes et déclarations des opérateurs, avant leur planification et affectation.

**Ordre de contrôle**
: Instruction formelle déclenchant la réalisation d'un contrôle sur un opérateur, une culture ou un lot. Planifié et affecté à un agent avant exécution.

### P

**Passeport phytosanitaire (PP)**
: Document officiel attestant qu'un végétal ou produit végétal satisfait aux exigences phytosanitaires pour sa libre circulation dans l'Espace européen.

**PGRP (Plan de Gestion des Risques Phytosanitaires)**
: Plan élaboré par un opérateur pour prévenir et gérer les risques liés aux organismes nuisibles. Soumis à approbation de la DQ.

**Plan de contrôle**
: Ensemble des objectifs de contrôle annuels déclinés à partir des 5 programmes-cadres du COP. Base de la programmation.

**Point de contrôle**
: Critère ou élément vérifié lors de la réalisation d'un contrôle, devant être qualifié par un collaborateur DQ.

**Pôle**
: L'une des 3 unités organisationnelles centrales de la DQ, encadrée par un manager de proximité.

**Pression de contrôle**
: Intensité et fréquence des contrôles appliqués à un opérateur ou une situation, modulées par l'analyse de risque et l'indice de confiance.

**Procédure**
: Document décrivant les étapes de réalisation d'une activité de contrôle, les acteurs en charge, et les outils/documents supports.

**Programmation**
: Déclinaison opérationnelle du plan de contrôle par région, définissant le volume et la nature des interventions à réaliser. Se déroule en deux temps : initiale (début d'année, fixe) et ajustée (au fil de l'eau).

**PQP (Plan de Qualité Produit)**
: Référentiel professionnel définissant les exigences qualité pour un produit semencier donné. Souvent associé à la certification ESTA.

### R

**Rapport d'évaluation / constat**
: Document produit par un inspecteur à l'issue d'un contrôle, fondant la décision administrative de la DQ.

**Référent technique national**
: Agent Entreprise X en charge de l'exécution du système qualité (amélioration continue) et de la proposition des décisions d'autorisation ou agréments.

**Référent technique régional**
: Agent Entreprise X supervisant les inspecteurs d'une antenne et assurant le premier niveau de révision des rapports de contrôle.

**Référentiel professionnel**
: Ensemble de certifications ou évaluations délivrées à la demande, attestant du respect de niveaux de qualité ou de bonnes pratiques. Non obligatoires pour la circulation ou la commercialisation.

### S

**Section professionnelle**
: Instance de dialogue interprofessionnel au sein d'Entreprise X, organisée par grands groupes d'espèces (9 sections). Composée de représentants des fédérations de la filière.

**Surveillance officielle**
: Ensemble des actions de suivi et de vérification exercées par la DQ sur les opérateurs, leurs personnels, laboratoires et productions, de façon continue.

### V

**Végétal local**
: Référentiel professionnel valorisant les semences et plants issus de variétés adaptées aux conditions locales.
